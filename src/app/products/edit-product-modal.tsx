...
export function EditProductModal({ 
  product, categories, onClose, onSave, loading 
}: { 
  product: Product
  categories: Category[]
  onClose: () => void
  onSave: (id: string, data: EditProductFormData) => Promise<void>
  loading: boolean
}) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center overflow-hidden p-4">
      ...
      <div className="relative z-20 w-full max-w-md bg-white rounded-t-[32px] md:rounded-[32px] h-[80vh] md:h-auto md:max-h-[90vh] flex flex-col overflow-hidden shadow-2xl enter-rise">
        <div className="flex flex-col items-center pt-3 pb-2 w-full bg-white shrink-0 rounded-t-[32px]">
          ...
          <div className="w-full flex justify-between items-center px-4 pt-3">
            ...
            <h1 className="text-lg font-bold text-slate-900 tracking-tight">Edit Barang</h1>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-5 no-scrollbar">
          ...
          <label className="flex flex-col w-full group">
            <span className="text-slate-700 text-sm font-semibold mb-1.5 ml-1">Satuan</span>
            <Input 
              className="w-full h-[50px] px-4 rounded-[14px] bg-[#FDF9F1] border-2 border-transparent text-slate-900 text-sm font-medium placeholder:text-slate-400 focus:outline-none focus:border-primary focus:ring-0 transition-colors" 
              placeholder="pcs"
              value={formData.unit}
        ...
        <div className="p-4 bg-white border-t border-slate-100 shrink-0 mt-auto pb-safe">
          <Button 
            ...
            disabled={loading}
            onClick={() => onSave(product.id, formData)}
          >
